<?php
/**
 * OSSeed WordPress Event Calendar Plugin
 *
 * @package WordPress
...
/**
 * Event Calendar button to post and page screens.
 *
 * @param mixed $context Get context value.
 * @return void
 */
...
function event_calendar_add_form_button_html() {
	...
	if (
		$screen->base === 'post'
		&& ( $screen->id === 'post' || $screen->id === 'page' )
		&& ( $screen->post_type === 'post' || $screen->post_type === 'page' )
		) {
		...
		<div class="wrap"><div>
			...
			<div style="padding:15px 15px 0 15px;">
				<select id="calendar_civicomponent_id">
					<option value=""><?php esc_html_e( 'Select a frontend element.', 'civicrm-wordpress' ); ?></option>
					<option value="event-calendar"><?php esc_html_e( 'Event Calendar', 'civicrm-wordpress' ); ?></option>
				</select>
			</div>
			<div style="padding:15px;">
				<input type="button" class="button-primary" value="Insert Form" id="eventcalendar-wp-insert-shortcode"/>&nbsp;&nbsp;&nbsp;
				<a class="button" style="color:#bbb;" href="#" onclick="tb_remove(); return false;"><?php esc_html_e( 'Cancel', 'civicrm-wordpress' ); ?></a>
			</div>
			</div>
		</div>
	</div>
 ...
 * Register the Event Calendar shortcode.
 *
 * @param mixed $atts Get atts value.
 * @return mixed
 */
